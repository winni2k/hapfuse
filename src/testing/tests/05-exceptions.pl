...
my $cmd =   "./hapfuse  -tGT -TGT,GP -o $results_vcf $sd/samples/test20/test20.chr20*.bin.vcf 2>&1";
print "Call: $cmd\n";
my $res = qx/$cmd/;
like($res, qr/Need to specify APP or GP as input tag if specifying GP as output tag/, "Missing for GP out");


$cmd =   "./hapfuse  -tGT -TGT,APP -o $results_vcf $sd/samples/test20/test20.chr20*.bin.vcf 2>&1";
print "Call: $cmd\n";
$res = qx/$cmd/;
like($res, qr/Need to specify APP as input tag if specifying APP as output tag/, "Missing for APP out");


$cmd =   "./hapfuse  -tGT,GP -TGT,APP -o $results_vcf $sd/samples/test20/test20.chr20*.bin.vcf 2>&1";
print "Call: $cmd\n";
$res = qx/$cmd/;
like($res, qr/Need to specify APP as input tag if specifying APP as output tag/, "Missing for APP out 2");


$cmd =   "./hapfuse  -tAPP -TGT,APP -o $results_vcf $sd/samples/test20/test20.chr20*.bin.vcf 2>&1";
print "Call: $cmd\n";
$res = qx/$cmd/;
